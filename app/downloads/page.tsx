...
export default function DownloadsPage() {
  return (
    <main className="bg-marinho">
      <Header />
      <PageHero titulo="Downloads" crumb="Download" imagem="/images/jeFiber_18.jpg" />

      <section className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        <p className="font-mono-tech text-xs uppercase text-papoula">Todos · {DOWNLOADS.length} arquivos</p>
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {DOWNLOADS.map((d) => (
            <a key={d.arquivo} href={`${BASE}${d.arquivo}`} target="_blank" rel="noopener noreferrer" download
              className="group flex items-center gap-4 rounded-xl border border-line/10 bg-marinho-2/40 p-5 transition-all hover:-translate-y-1 hover:border-papoula/50 hover:shadow-[0_18px_40px_rgba(0,0,0,.35)]">
              <span className="grid h-14 w-14 shrink-0 place-items-center rounded-xl border border-line/10 bg-marinho">{IcPdf}</span>
              <span className="min-w-0 flex-1">
                <span className="block truncate font-display text-base font-bold text-ink">{d.nome}</span>
                <span className="font-mono-tech text-xs uppercase text-ink/50">PDF · {d.tamanho}</span>
              </span>
